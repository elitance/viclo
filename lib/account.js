        ...
        if (req.session.pw === req.body.pw) {
            db.query('select * from account where un = ?',[req.body.un],(err,data) => {
                if (!data[0] || data[0].un === req.session.un) {
                    db.query('update account set pw = ?, un = ? where un = ?',[template.crypto(req.body.nPw),req.body.un,req.session.un],(err,data) => {
...
router.post('/login',(req,res) => {
    db.query('select * from account where un = ? and pw = ?',[req.body.un,template.crypto(req.body.pw)],(err,data) => {
        if (!data[0]) {
