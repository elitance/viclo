...
router.post('/',(req,res) => {
    db.query('update account set un = ? where un = ?',[req.body.un,req.cookies.un],(err,data) => {
        res.cookie('un',req.body.un,{httpOnly: true,maxAge: 9000000});
        db.query('update docs set author = ? where author = ?',[req.body.un,req.cookies.un],(err,data) => {
            db.query('select * from account where un = ?',[req.body.un],(err,data) => {
                if (req.body.nPw !== '') {
                    if (data[0].pw === req.body.pw) {
                        db.query('update account set pw = ? where un = ?',[req.body.nPw,req.body.un],(err,data) => {
                            res.redirect('/');
                        });
                    } else if (data[0].pw !== req.body.pw) {
                        res.redirect('/account/verify?root=manage');
                    }
                } else {
                    res.redirect('/');
                }
...
router.get('/verify',(req,res) => {
    const query = url.parse(req.url,true).query;
    if (query.root === 'del') {
        res.send(template.verify('/account/delete',`<input type='hidden' name='un' value='${req.body.un}'><span class="fail">Password is incorrect. Try again.</span>`));
    } else if (query.root === 'manage') {
        res.send(template.verify('/account',`<input type='hidden' name='un' value='${req.body.un}'><input type='hidden' name='nPw' value='${req.body.nPw}'><span class='fail'>Password is incorrect. Try again.</span>`));
    }
})
...
router.post('/verify',(req,res) => {
    if (req.body.manage) {
        ...
        res.send(template.verify('/account/delete',`<input type='hidden' name='un' value='${req.body.un}'>`));
    }
});
...
router.post('/delete',(req,res) => {
    db.query('select * from account where un = ?',[req.body.un],(err,data) => {
        if (data[0].pw === req.body.pw) {
            ...
            res.redirect('/account/verify?root=del');
        }
    });
})
