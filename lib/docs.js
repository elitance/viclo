...
router.get('/write',(req,res) => {
    ...
    res.send(template.html('Write',template.accLink(req.cookies.un),template.part('write')));
});

router.post('/write',(req,res) => {
    db.query('insert into docs (title,content,author,keywords) values (?, ?, ?, ?)',[req.body.title,req.body.content,req.cookies.un,req.body.keywords.toLowerCase()],(err,data) => {
        if (err) throw err;
...
router.get('/:page',(req,res) => {
    db.query('select * from docs where id = ?',[req.params.page],(err,docs) => {
        if (!docs[0]) {
            ...
            res.send(template.html(docs[0].title,template.accLink(req.cookies.un),template.part('doc',[docs[0].title,content,docs[0].author,docs[0].id])));
        }
    });
});
...
router.get('/update/:page',(req,res) => {
    db.query('select * from docs where id = ?',[req.params.page],(err,doc) => {
        ...
        res.send(template.html('Update',template.accLink(req.cookies.un),`
...
router.post('/delete',(req,res) => {
    if (!req.cookies.un) res.redirect('/account/login');
